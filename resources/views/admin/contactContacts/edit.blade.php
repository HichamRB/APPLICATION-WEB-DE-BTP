...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contact-contacts.update", [$contactContact->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group col-md-6">
                <label for="job_title">{{ trans('cruds.contactContact.fields.job_title') }}</label>
                <input class="form-control {{ $errors->has('job_title') ? 'is-invalid' : '' }}" type="text" name="job_title" id="job_title" value="{{ old('job_title', $contactContact->job_title) }}">
                @if($errors->has('job_title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('job_title') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.contactContact.fields.job_title_helper') }}</span>
            </div>
            <div class="form-group col-md-6">
