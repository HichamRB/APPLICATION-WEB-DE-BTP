        ...
        <form method="POST" action="{{ route("admin.projects.store") }}" enctype="multipart/form-data">
          ...
          <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control date {{ $errors->has('order_date') ? 'is-invalid' : '' }}" type="text" name="order_date" id="order_date" value="{{ old('order_date', '') }}">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('project_chef') ? 'is-invalid' : '' }}" name="project_chef_id" id="project_chef_id">
                    @foreach($project_chefs as $id => $entry)
                        <option value="{{ $id }}" {{ old('project_chef_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
