    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contact-companies.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\ContactCompany::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 's') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('rib') ? 'is-invalid' : '' }}" type="text" name="rib" id="rib" value="{{ old('rib', '') }}">
                @if($errors->has('rib'))
                    <div class="invalid-feedback">
                        {{ $errors->first('rib') }}
                    </div>
