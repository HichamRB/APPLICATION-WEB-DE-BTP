    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sites.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('site_manager') ? 'is-invalid' : '' }}" name="site_manager_id" id="site_manager_id">
                    @foreach($site_managers as $id => $entry)
                        <option value="{{ $id }}" {{ old('site_manager_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('progress_level') ? 'is-invalid' : '' }}" type="number" name="progress_level" id="progress_level" value="{{ old('progress_level', '') }}" step="1">
